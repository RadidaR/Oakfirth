    ...
    void DrawDynamicInspector(Material material, MaterialEditor materialEditor, MaterialProperty[] props)
    {
        ...
        else
        {
            for (int i = 0; i < props.Length; i++)
            {
                ...
                if (!material.HasProperty("_render_normals_options") || shaderName.Contains("Mobile"))
                {
                ...
                {
                    if (material.GetInt("_RenderClip") == 0 && prop.name == "_RenderCoverage")
                        continue;
                    if (material.GetInt("_RenderClip") == 0 && prop.name == "_AlphaCutoffValue")
                        continue;
                    if (material.GetInt("_RenderClip") == 0 && prop.name == "_AlphaSharpnessValue")
                        continue;
                ...
                if (material.HasProperty("_SubsurfaceValue"))
                {
                    ...
                    if (prop.name == "_TransStrength")
                        continue;
                    if (prop.name == "_TransNormal")
                        continue;

                    if (material.GetTag("RenderPipeline", false) == "HDRenderPipeline" || shaderName.Contains("Standard") || shaderName.Contains("Simple"))
                    {
                        if (prop.name == "_SubsurfaceNormalValue")
                            continue;
                        if (prop.name == "_SubsurfaceDirectValue")
                            continue;
                        if (prop.name == "_SubsurfaceAmbientValue")
                            continue;
                        if (prop.name == "_SubsurfaceShadowValue")
                            continue;
                    }
                }
                else
                {
                    ...
                    if (prop.name == "_SubsurfaceScatteringValue")
                        continue;
                    if (prop.name == "_SubsurfaceAngleValue")
                        continue;
                    if (prop.name == "_SubsurfaceNormalValue")
                        continue;
                    if (prop.name == "_SubsurfaceDirectValue")
                ...
                if (material.HasProperty("_MotionValue_20") || material.HasProperty("_MotionValue_30"))
                {
                ...
                if (!hasMotionLocals)
                {
                    if (prop.name == "_SpaceMotionLocals")
                        continue;
                }
                ...
                if (material.HasProperty("_MaterialType") && material.shader.name.Contains("Objects"))
                {
                    if (prop.name == "_MaterialType")
                        continue;
                    if (prop.name == "_MessageImpostorPivots")
                        continue;
                }
